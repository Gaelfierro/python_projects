def Story(game):
    ...
    if user_input==1:
        ...
        user_input=int(raw_input('...'))
        if user_input ==1:
            print'You ate the berries, they tasted weird, it turns out that they were indeed poisonous and you died!'
        elif user_input==2:
            print'The mushrooms were poisonous, you got diarrhea and you died!'
        else:
            print'Good thing you avoided those, they were both really poisonous.'
    elif user_input==2:
        print 'There is a cave 2 miles away and cabin on top of the mountain.'
        ...
        user_input=int(raw_input('...'))
        if user_input==1:
            print 'After an hour of walking, you finally get to the river, you reach out to drink some water, but massive fish came out and ate your face, killing you in the process.'
        elif user_input==2:
            print 'Good choice, the fish there are as large as a cow (which is odd, because the river is rather small, maybe it is deeper then it looks.) and can eat a human in one bite.'
            print 'press 1 to go back to the beginning.'

def The_Room(room):
    print 'You wake up in an empty room with two doors, one in the left, one in the right, and on the front a large window.'
    ...
    if user_input==1:
        print 'You tried to to open the door, but the darn thing is locked, maybe there is another way to open it...'
    elif user_input==2:
        print 'You open the right door, there is a sink, a toilet, a bathtub, a large hole in the wall, and cabinet with a mirror on top of the sink. A bathroom...'
        ...
        if user_input==1:
            ...
            print 'The hallway ends with a large dark green door, but before you could reach the dooknob, you remember that you noticed something strange about one particular painting.'
            print 'So what will you do now? Open the door (press 1) or go back to inspect the painting (press 2).'
        elif user_input==2:
            print 'you look through the window and it seems that you are on a large building. Trying to jump will lead to a horrible death, so you leave it alone.'
